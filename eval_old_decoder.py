...
def beam_evaluate(data_name, checkpoint_file, data_folder, beam_size, outdir, graph_feature_dim=512, dataset='TEST'):
	...
	for caption_idx, (image_features, obj, rel, obj_mask, rel_mask, pair_ids, caps, caplens, orig_caps) in enumerate(
		...
		while True:
			...
			# embeddings = decoder.embedding_bn(embeddings)
			h1, c1 = decoder.top_down_attention(
				torch.cat([h2, image_features_mean, graph_features_mean, embeddings], dim=1),
...
if __name__ == '__main__':
	...
	parser.add_argument('--outdir',
	                    default='outputs/server/cascade_img_first_contextGAT/batch_size-512_epochs-52_dropout-0.5_patience-20_stop-metric-Bleu_4_aug-3_edgeprob-0.2_nodeprob-0.2_attrprob-0.2/emb-1024_att-1024_dec-1024/cgat_useobj-True_userel-True_ksteps-1_updaterel-True/seed-1/',
	                    type=str,
	                    help='path to location where the outputs are saved, so the checkpoint')
	parser.add_argument('--checkpoint_file', type=str, default='checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar',
